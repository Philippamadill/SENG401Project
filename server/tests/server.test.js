...
describe('Database Connection', () => {
  ...
  it('should attempt to connect to the database', () => {
    expect(databaseConnection.connect).toBeTruthy();
  });

  it('should return if failed to connect to the database', () => {
    const spy = jest.spyOn(databaseConnection, 'connect');
...
describe('Book queries', () => {
  ...
  it('status 404 received when creating book without req fields', async () => {
    const newBook = {};
    const res = await request(app).get('/createBook').query({newBook});
    expect(res.status).toEqual(404);
  });

  it('status 400 received when ISBN not provided', async () => {
    const res = await request(app).get('/book/getBookByISBN');
    expect(res.status).toEqual(400);
  });

  it('status 500 received when internal server error', async () => {
    const res = await request(app).get('/book/getBookByISBN').query({ISBN: "null"});
    expect(res.status).toEqual(500);
  });

  it('status 404 received when deleting book with invalid ISBN', async () => {
...
describe('Bookshelf queries', () => {
  ...
  it('status 404 received when adding book to currently reading without ISBN or username', async () => {
    const res = await request(app).get('/bookshelf/addBookToCurrentlyReading');
    expect(res.status).toEqual(404);
  });

  it('status 404 received when adding book to want to read without ISBN or username', async () => {
    const res = await request(app).get('/bookshelf/addBookToWantToRead');
    expect(res.status).toEqual(404);
  });

  it('status 404 received when adding book to already read without ISBN or username', async () => {
...
describe('Review queries', () => {
  ...
  it('status 400 received when getting review without ISBN', async () => {
    const res = await request(app).get('/review/getReviewsByISBN');
    expect(res.status).toEqual(400);
  });

  it('status 400 received when getting review without username', async () => {
    const res = await request(app).get('/review/getReviewsByUsername');
    expect(res.status).toEqual(400);
  });

  it('status 404 received when deleting review without review_id', async () => {
